...
# ======================================================
# COMMON USER JOB QUERY
# ======================================================

def user_jobs(request):
    return ETLJob.objects.filter(
        created_by=request.user
    ).order_by("-created_at")
...
def engineer_dashboard(request):
    ...
    context = {
        "total_sources": APISource.objects.filter(created_by=request.user).count(),
        "total_jobs": ETLJob.objects.filter(created_by=request.user).count(),
        "successful_jobs": ETLJob.objects.filter(
            created_by=request.user,
            status="Completed"
        ).count(),
    ...
    if request.method == "POST":
        ...
        try:
            ...
            send_mail(
                ...
                message=f"Pipeline failed for job: {job.job_name}\n\nError: {str(e)}",
                from_email=settings.DEFAULT_FROM_EMAIL,
                recipient_list=[request.user.email],
                fail_silently=True,
            )

            messages.error(request, "Pipeline Execution Failed")
...
def sync_warehouse(request, id):
    ...
    messages.success(
        request,
        "Data Warehouse Synchronization Completed Successfully"
    )
